...
- **v19 Lag-MLP (baseline / submission)**
  - Train: `../.venv/bin/python scripts/train_model.py --target_mode level`
...
- **Level+Residual Blend (submission option)**
  - Train residual copy: `../.venv/bin/python scripts/train_model.py --target_mode residual --prefix lag_mlp_residual`
  - Inference blend (alpha=0.6 best so far) lives in `submissions/solution_blend.py`; to submit, copy to `solution.py` before zipping.
  - Leaderboard: LB 0.3571 (best to date).

- **CatBoost (offline oracle)**
  - Run CV: `../.venv/bin/python scripts/train_catboost_experiment.py --save_model_dir models --save_prefix catboost_v19 --save_importance`
  - Feature importances only (no retrain) for a saved model: `../.venv/bin/python scripts/dump_catboost_importance.py --model_path models/catboost_v19_fold0.cbm --out_path cat_importance.csv`
  - Not a submission candidate (slower, lower CV than MLP).

- **LSTM baseline (raw 32-dim)**
  - Quick pilot (subset): `../.venv/bin/python scripts/train_lstm_experiment.py --window 30 --hidden 256 --layers 2 --lr 5e-4 --epochs 10 --subset 120000`
  - Outcome: Val R² ~0.31; not beating v19. Keep as reference.

- **Micro-Mamba (SSD-style, CPU-friendly)**
  - Pilot: `../.venv/bin/python scripts/train_mamba_experiment.py --window 30 --d_model 64 --d_state 32 --layers 2 --d_conv 4 --subset 120000 --residual_target`
...
- **LSTM submission (raw window)**
  - Train + save: `../.venv/bin/python scripts/train_lstm_experiment.py --window 30 --hidden 256 --layers 2 --lr 5e-4 --epochs 20 --save_path models/lstm_submission.pth --save_norm models/lstm_submission_norm.npz --save_meta models/lstm_submission_meta.json`
  - Streaming code: `submissions/solution_lstm.py` loads the saved artifacts; copy to `solution.py` before zipping if you want to submit this variant.
...
- Ensure `solution.py` at repo root with the desired model (copy `submissions/solution_blend.py` over if submitting blend).
