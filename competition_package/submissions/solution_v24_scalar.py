...
# --- Configuration ---
ALPHA = 0.55 # Weight for Level Model (v19)
# Weight for Residual Model (v21) will be 1 - ALPHA = 0.45
...
class PredictionModel:
    def __init__(self):
        ...
        if not os.path.exists(norm_v19_path):
             raise FileNotFoundError(f"v19 Norm not found: {norm_v19_path}")
        ...
        # Reconstruct v19 arch from checkpoint dims
        input_dim_v19 = ckpt_v19["input_dim"]
        hidden_dim_v19 = ckpt_v19["hidden_dim"]
        output_dim_v19 = ckpt_v19["output_dim"]
        
        for fname in fold_files_v19:
            path = os.path.join(models_dir, fname)
            ckpt = torch.load(path, map_location=self.device)
            m = LagMLP_v19(input_dim_v19, hidden_dim_v19, output_dim_v19)
        ...
        if not os.path.exists(norm_v21_path):
             raise FileNotFoundError(f"v21 Norm not found: {norm_v21_path}")
        ...
        input_dim_v21 = ckpt_v21["input_dim"]
        hidden_dim_v21 = ckpt_v21["hidden_dim"]
        output_dim_v21 = ckpt_v21["output_dim"]
        
        for fname in fold_files_v21:
            path = os.path.join(models_dir, fname)
            ckpt = torch.load(path, map_location=self.device)
            m = LagMLP_v21(input_dim_v21, hidden_dim_v21, output_dim_v21)
            m.load_state_dict(ckpt["state_dict"])
            m.eval()
