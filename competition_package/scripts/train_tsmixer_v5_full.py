...
EMA_DECAY = 0.999
USE_Z_DELTA = True
REDUCE_CHANNELS = False  # If True, drop slope/skew blocks → 128 channels
...
def build_hybrid_dataset(df: pd.DataFrame, clip_min: np.ndarray, clip_max: np.ndarray):
    ...
    for seq_ix, df_seq in df.groupby("seq_ix"):
        ...
        for idx in range(T - 1):
            ...
            if USE_Z_DELTA:
                # Scale-invariant deltas to reduce sensitivity to level shifts
...
def train_one_fold(X_train, y_train, X_val, y_val, fold_idx, global_mean, global_std):
    ...
    for epoch in range(N_EPOCHS):
        ...
        preds, targets = [], []
        with torch.no_grad():
            # Evaluate EMA weights for stability
